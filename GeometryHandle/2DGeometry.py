...
def triangulate_polygon(points: list = [], tolerance: float = 0.0, tolerance_col: float = 0.0) -> tuple[list, list]:
    ...
    # 2. Use Delaunay triangulation method
    triangle_indexes = Delaunay(points).simplices

    # 3. Remove triangles outside the concave boundary
    centroids = np.mean(points[triangle_indexes], axis=1)
